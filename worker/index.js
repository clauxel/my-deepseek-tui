...
const planCatalog = {
  starter: {
    ...
    currency: 'USD',
    summary: 'personal remote DeepSeek-TUI running',
  },
  pro: {
    id: 'pro',
    name: 'Pro',
    ...
    currency: 'USD',
    summary: 'more concurrency, persistent logs, and private workspaces',
  },
  team: {
    id: 'team',
    name: 'Team',
    ...
    currency: 'USD',
    summary: 'shared projects, member permissions, and audit records',
  },
}
...
const indexablePaths = [
  ...
  '/deepseek-tui-team-workspace',
  '/deepseek-tui-remote-runner',
  '/deepseek-tui-alternative',
  '/deepseek-tui-pricing',
  '/deepseek-tui-ssh-agent',
  '/deepseek-tui-audit-logs',
  '/privacy',
  '/terms',
]
...
function polarCheckoutOptions(requestUrl) {
  return {
    plans: planCatalog,
    defaultPlanId: 'pro',
...
function resolveConfiguredProductId(env, planId, billing) {
  ...
  const tier = planId === 'team' ? 'TEAM' : planId === 'starter' ? 'STARTER' : 'PRO'
  const normalizedSelection = normalizeEnvKey(`${planId}_${billing}`)
  const keys = [
    `POLAR_PRODUCT_${tier}_${cycle}`,
    `POLAR_PRODUCT_ID_DEEPSEEK_TUI_${normalizedSelection}`,
...
async function getOrCreatePolarProduct(env, apiKey, plan, billing, successUrl) {
  ...
  const product = await requestPolarJson(apiKey, `${resolvePolarBase(env)}/v1/products`, {
    name: `DeepSeek-TUI Cloud ${plan.name} (${billingLabel})`,
...
async function handleCheckout(request, env, requestUrl) {
  ...
  if (!apiKey) {
    return handlePolarCheckout(request, env, polarCheckoutOptions(requestUrl))
  }
...
async function fetchAsset(request, env) {
  ...
  if (requestUrl.pathname === '/api/analytics/events') {
    return handleAnalyticsRequest(request, env, { siteKey: 'deepseek-tui' })
  }
