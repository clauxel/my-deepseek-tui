...
function normalizeOrigin(origin: string) {
  try {
    ...
    return typeof window !== 'undefined' ? window.location.origin : 'https://deepseek-tui.space'
  }
}
...
export function buildSeoDocument(args: {
  ...
  if (routeView === 'privacy') {
    ...
    const description = 'How DeepSeek-TUI Cloud handles checkout, analytics, support, and workspace onboarding data.'
    return {
      title,
      description,
      canonicalUrl,
  ...
  if (routeView === 'terms') {
    ...
    const description = 'Terms for using DeepSeek-TUI Cloud checkout, hosted workspace onboarding, and support.'
    return {
      title,
      description,
      canonicalUrl,
  ...
  return {
    ...
    description: 'The requested DeepSeek-TUI Cloud page was not found.',
    canonicalUrl,
    robots: 'noindex,nofollow',
